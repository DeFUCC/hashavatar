...
import HashVueAvatar from './components/hash-vue-avatar.vue';
...
useFavicon(() => hashAvatar?.({ pub: state.pub, size: 256, dark: state.options.dark }))

const { share, isSupported: isShareSupported } = useShare()

const sharePNG = async () => {
	const avatarUrl = hashAvatar({
		pub: state.pub,
		size: 512,
...
Layout.overflow-hidden
	template(#home-hero-before)
		img.absolute.top-0.bottom-0.left-0.right-0.bg-red.w-full.op-10(inert :key="state.pub" :src="hashAvatar({pub:state.pub, svg:false, size:1400,  draw:'squares', reflect: false, dark:state.options.dark, round: false,})" )
		...
		.flex(@click="state.generatePair()")
			transition(name="fade")
				.flex.items-center.image-src(
					...
					hash-vue-avatar.rounded-full.absolute.transform.scale-200.filter.blur-40px.z-5.op-80.dark-op-30(
						:pub="state.pub"
						:size="300"
						:svg="true"
						)
					hash-vue-avatar.rounded-full.absolute.transform.scale-110.filter.blur-30px.z-10.dark-op-70(
						:pub="state.pub"
						:size="300"
						:svg="true"
						)
					hash-vue-avatar.rounded-full.cursor-pointer.z-20.shadow-2xl(
						:pub="state.pub"
						:size="300"
						:embed="state.pair"
						:svg="false"
	...
	template(#home-features-after)
		.my-16.w-full.min-h-100vh.bg-cover.bg-fixed.flex.items-center.justify-center.flex-wrap.relative(
			...
			//- :style="{backgroundImage:`url(${hashAvatar({pub:state.pub, size: 1400, draw:'squares', reflect: false, dark:state.options.dark, round: false})})`}"
			transition(name="fade")
				object.absolute.top-0.left-0.right-0.bottom-0.z-1.h-full.w-full(
						:key="state.pub"
						type="image/svg+xml"
						:data="hashAvatar({pub:state.pub, svg:'interactive', size:1200,  draw:'squares', reflect: false, dark:state.options.dark, round: false})")
			.h-80.w-80.min-h-70.z-4
				transition(name="fade" mode="out-in")
					object.shadow-2xl.rounded-full.z-2.w-full(
						:key="state.pub"
						type="image/svg+xml"
						:data="hashAvatar({pub:state.pub, svg:'interactive', size: 200, dark:state.options.dark})")
				...
				transition-group(name="fade")
					.flex(
						...
						object.rounded-full.cursor-pointer.shadow-lg.w-60px.h-60px(
							...
							:data="hashAvatar({pub:rec.snapshot.pub, svg:'interactive', size: 60})"
